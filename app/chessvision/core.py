...
class ChessVision:
    ...
    def _initialize_board_extractor(self):
        if self._board_extractor is not None:
            logger.debug("✅ Board extractor already initialized.")
            return

        logger.info("🔧 Initializing YOLOv8 extractor...")

        checkpoint_path = self._board_extractor_path
        if not checkpoint_path or not os.path.exists(checkpoint_path):
            raise FileNotFoundError(f"❌ Board extractor weights not found: {checkpoint_path}")

        try:
            model = YOLO(checkpoint_path)
            logger.info(f"✅ YOLOv8 model loaded successfully: {type(model)}")
            self._board_extractor = model.predict 
    ...
    def extract_board(
        ...
        image_batch = torch.Tensor(np.array([comp_image])) / 255.0
        image_batch = image_batch.permute(0, 3, 1, 2).to(self._device)

        # Get model predictions
        with torch.no_grad():
            results = self.board_extractor(image_batch, verbose=False)
    ...
    def classify_position(
        ...
        batch = torch.Tensor(squares).permute(0, 3, 1, 2).to(self._device)
        batch /= 255.0
    ...
    def image_to_fen(self, image_path: str, threshold: float = 0.5, flip: bool = False) -> str:
        """High-level method to directly get FEN from image path."""
        image = cv2.imread(image_path)
        if image is None:
            raise FileNotFoundError(f"Could not read image at: {image_path}")

        result = self.process_image(image, threshold=threshold, flip=flip)
        if result.position is None or result.position.fen is None:
            raise ValueError("Could not extract valid FEN from image")

        return result.position.fen
